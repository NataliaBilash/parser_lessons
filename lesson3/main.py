...
def get_data(url):
    ...
    for item in range(1, 24):
        ...
        folder_name = f"/home/zeroff/Рабочий стол/parcer/lesson3/data/data_{item}"
        ...
        with open(f"/home/zeroff/Рабочий стол/parcer/lesson3/{folder_name}/projects_{item}.html", "w") as file:
            file.write(req.text)

        with open(f"/home/zeroff/Рабочий стол/parcer/lesson3/{folder_name}/projects_{item}.html") as file:
            src = file.read()

        soup = BeautifulSoup(src, "lxml")
        articles = soup.find_all("article", class_="ib19") #выбираем карточек

        projects_urls = [] 
        for article in articles: #ЗАБИРАЕМ СЫЛКИ ИЗ КАРТОЧЕК
            project_url = "http://www.edutainme.ru" + article.find("div", class_="txtBlock").find("a").get("href")
            projects_urls.append(project_url)
        
        #ТЕПЕРЬ надо перти по каждой ссылке и собртаь данные
        for project_url in projects_urls:
            req = requests.get(project_url, headers)
            project_name = project_url.split("/")[-2]

            with open(f"/home/zeroff/Рабочий стол/parcer/lesson3/{folder_name}/{project_name}.html", "w") as file:
                file.write(req.text)

            with open(f"/home/zeroff/Рабочий стол/parcer/lesson3/{folder_name}/{project_name}.html") as file:
                src = file.read()
    ...
    with open("/home/zeroff/Рабочий стол/parcer/lesson3/data/projects_data.json", "a", encoding="utf-8") as file:
